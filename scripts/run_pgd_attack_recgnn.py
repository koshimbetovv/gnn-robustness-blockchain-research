...
from src.attacks.pgd_recgnn import RecGNNPGDAttack
...
# controllable by perturbing the target node's own feature vector, so they
# are excluded from PGD. `None` selects the dataset-appropriate default.
ATTACK_DIM = None
...
def pick_targets_graph(
    ...
    """Pick target positions among labeled nodes in a per-timestep graph."""
...
def main():
    ...
    # Load data first using the checkpoint's data config so it matches training.
    ...
    if DATASET == "elliptic":
        ...
        default_attack_dim = 93  # 93 local standardized + 2 ANF = 95 total
        ...
        default_attack_dim = sequence.num_features - 2  # drop trailing 2 ANF columns
    else:
        raise ValueError(
    ...
    # model_loader builds RecGNN from config.json (needs state_rows).
    ...
    atk = RecGNNPGDAttack(model, device, attack_dim=attack_dim, clamp=CLAMP)
    ...
    for graph in sequence.test_graphs:
        ...
        if int(labeled_mask.sum().item()) == 0:
            # Advance clean state and skip.
            atk.attack_step(x, edge_index, torch.empty(0, dtype=torch.long, device=device),
        ...
        # First call: clean pass to pick targets. `attack_step` with empty
        # targets advances state along the clean trajectory without perturbing.
        # We'd then re-call attack_step with targets — but that would re-advance
        # state. Instead, do the full pick+attack in one pass by extracting
        # clean logits from a preview forward.
        #
        # Simpler: just call attack_step once with the targets we computed from
        # a no-grad preview of log-probs using the current saved state.

        # Compute clean preview (state unaffected — attack_step saves/restores).
        # Here we use a no-grad forward that mutates state, then restore it.
        ...
        if targets.numel() == 0:
            # Still need to advance the clean state for the next timestep.
            _x_adv, log_probs_adv, log_probs_clean = atk.attack_step(
                x, edge_index,
                torch.empty(0, dtype=torch.long, device=device),
                torch.empty(0, dtype=torch.long, device=device),
                eps=EPS, alpha=ALPHA, steps=STEPS, random_start=RANDOM_START,
            ...
            # Pool clean so concat baseline is complete.
        ...
        x_adv, log_probs_adv, log_probs_clean = atk.attack_step(
        ...
        # Metrics computed on labeled nodes only.
        ...
        # Build attack mask over labeled positions.
        ...
        # L2 perturbation on successful flips (restrict to perturbable slice [:attack_dim]).
        clean_rows = x[targets, :atk.attack_dim]
        adv_rows = x_adv[targets, :atk.attack_dim]
    ...
    print()
    print(f"PGD RecGNN | eps={EPS} alpha={ALPHA} steps={STEPS} random_start={RANDOM_START}")
