...
def main():
    if MODEL_NAME not in STATIC_MODELS:
        raise NotImplementedError(
            f"{MODEL_NAME!r} is not a static-feature model supported by this PGD driver. "
    ...
    x_adv = atk.attack(targets, eps=EPS, alpha=ALPHA, steps=STEPS, random_start=RANDOM_START)
    ...
    # Restrict to the perturbable slice [:, :atk.attack_dim] since the PGD
    ...
    print()
    print(f"PGD | eps={EPS} alpha={ALPHA} steps={STEPS} random_start={RANDOM_START}")
