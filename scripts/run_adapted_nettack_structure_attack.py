...
# Adapted-NETTACK structure-only threat model:
#   N_STRUCT  : maximum number of edge ADDITIONS per target (no deletions, no
#               feature perturbations). Passing eps_feat=0.0 deactivates the
#               continuous L2 feature step in AdaptedNettackAttack via its
#               existing `feat_score > 0.0` guard.
N_STRUCT = 2
...
def make_run_dir(model_name: str):
    """Create attacks/model_adapted_nettack_structure_YYYYMMDD_HHMMSS/ and return (run_dir, timestamp)."""
    repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    ts = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    run_dir = os.path.join(repo_root, "attacks", f"{model_name}_adapted_nettack_structure_{ts}")
...
def main():
    ...
    # Structure-only attack: feature pipeline is irrelevant for the perturbation
    # itself, but we still need ChronoWaveGNN's derived slice present so its
    # forward pass works on the unperturbed feature tensor.
    ...
    # Structure-only: pass eps_feat=0.0 and rebuild_fn=None. attack_dim is left
    # unset; the L2 feature step is gated off inside _attack_single_target.
    ...
    x_adv, edge_index_adv = atk.attack(targets, n_struct=N_STRUCT, eps_feat=0.0)
    attack_time_seconds = float(time.perf_counter() - t_start)

    # Sanity: structure-only must leave x untouched.
    assert torch.equal(x_adv, data.x), "Structure-only driver should not perturb x."

    # NOTE (vs. FGSM/PGD): the victim is queried on the perturbed edge_index,
    # because Adapted-NETTACK adds graph edges (no feature deltas here).
    with torch.no_grad():
    ...
    config = {
        "timestamp": ts,
        "attack": "AdaptedNETTACK-Structure",
