...
class RecGNNFGSMAttack:
    """FGSM for RecGNN, operating one timestep at a time within a sequence.
    ...
    Protocol per test timestep:
      ...
      3. Restore pre-state; run FGSM gradient forward on a small `delta`.
    ...
    def attack_step(
        ...
        """Run FGSM on the current timestep.
        ...
        direction = +1.0
        delta = torch.zeros(
            (target_idx.numel(), self.attack_dim),
            device=self.device,
            requires_grad=True,
        )
        ...
        self._restore_state(snap_pre)
        x_out = self._apply_delta(x, target_idx, delta_adv.detach())
